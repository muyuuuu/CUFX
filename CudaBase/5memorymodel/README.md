...
> 假设我们在大三学过操作系统的段页式内存管理，在大二学过计算机组成原理的 DMA。简单回忆一下：页式内存会在主存不够时被系统换到硬盘，由此会引发缺页中断；DMA 可以完成数据传输而不必引起 CPU 中断。

`cudaMemcpy` 是指设备端和主机端的数据相互拷贝。而 `cudaMalloc` 分配的内存是分页内存，当从主机的可分页内存拷贝数据到设备时，cuda 驱动程序首先分配临时的页面锁定内存，将可分页内存复制到页面锁定内存中，然后再从页面锁定内存传输到设备内存。 

与分页内存相反的是锁页内存，指在主机端上分配的一种特殊内存，这种内存在被分配后，其物理页面会被锁定在内存中，不会被操作系统交换到虚拟内存（位于硬盘）上。换句话说，锁页内存是一种不会被页面调度器换出到磁盘的物理内存。

- 对于分页内存，device 需要访问分页内存的每一页，将其拷贝到锁页内存缓冲区，然后再将其通过 DMA 一页页拷贝到 device 上。如果遇到缺页中断，还会引来额外的开销。
- 对于锁页内存，device 可以通过 DMA 直接获取内存数据，避免缺页中断以及节省从分页内存拷贝到锁页内存的时间。但是锁页内存会占用系统的物理内存，因此当物理内存较少时，过多地使用锁页内存可能会导致系统性能下降。其次，由于锁页内存不会被换出到磁盘，因此一旦程序崩溃或退出，锁页内存中的数据可能会丢失。

## 锁页内存

CUDA 提供两种方式在主机端分配锁页内存：
...
- cudaHostAllocMapped，此标志下分配的锁页内存可实现零拷贝功能，主机端和设备端各维护一个地址，通过地址直接访问该块内存，无需传输。
...
即无需拷贝就可以在主机端和设备端直接访问的内存。

- 当设备内存不足时可以利用主机内存。当需要比显存容量大的内存可以考虑。
- 避免主机和设备间的显式数据传输，注意，零拷贝并不是无需拷贝，而是无需显式拷贝。但隐式拷贝是异步的，可以和计算并行，隐藏内存传输延时。

对于零拷贝内存，使用 cudaHostAlloc 接口分配到主机内存中，并设置 cudaHostAllocMapped 标志，从而获取一个 host 端的指向被分配内存的指针，再调用 cudaHostGetDevicePointer 来获取一个 device 端的指针来指向相同的内存区域，而 CUDA 4.0 引入的 UVA(Unified Virtual Addressing) 则省去了调用 cudaHostGetDevicePointer 的步骤，由 CUDA 来完成这一步骤，也就是说内部执行是一样的，只是代码被简化了。
...
如果不考虑 `cudaMemcpy`，那么传统内存最快；当数据量较大时，`cudaMallocManaged` 会更快，因为 `cudaMemcpy` 开销较大。
